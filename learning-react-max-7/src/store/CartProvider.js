...
const cartReducer = (curState, action) => {
    ...
    if (action.type === 'ADD') {
        const totalAmount = curState.totalAmount + action.item.price * action.item.amount;
        const existingCartItemIdx = curState.items.findIndex(item => item.id === action.item.id);
    ...
    if (action.type === 'REMOVE') {
        const existingItemIdx = curState.items.findIndex(item => item.id === action.id);
        ...
        if (existingItem.amount === 1) {
            ...
            items = curState.items;
            items[existingItemIdx] = { ...existingItem, amount: existingItem.amount - 1 };
        }
        return {
            items,
            totalAmount
